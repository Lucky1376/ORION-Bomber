...
def SPC(ip, port, login=None, password=None):
    ...
    ip_user = requests.get("http://icanhazip.com/",
                           headers={'User-Agent': ua}).text
    if login == None and password == None:  # проверка публичных прокси
        try:
            ip_proxy = requests.get(
                "http://icanhazip.com/", headers={'User-Agent': ua}, proxies=proxy_http, timeout=5)
...
class Proxy:
    ...
    def get(self):
        ...
        can = False
        try:
            result = requests.get("https://proxylist.geonode.com/api/proxy-list?limit=50&page=1&sort_by=lastChecked&sort_type=desc&speed=medium&protocols=http%2Chttps",
            ...
            while i < col_page:
                try:
                    response = requests.get(
                        f"https://proxylist.geonode.com/api/proxy-list?limit=50&page={i}&sort_by=lastChecked&sort_type=desc&speed=medium&protocols=http%2Chttps", headers={"User-Agent": ua}, timeout=15)
        ...
        for ct in url_list:
            can = False
            try:
                response = requests.get(url_list[ct], headers={
            ...
            if response.status_code == 200 or can == True:
                ...
                try:
                    all_list_bs = html.find(
                        "div", class_="table_block").find("tbody")
        ...
        try:
            response = requests.get(
                "https://www.sslproxies.org", headers={"User-Agent": ua}, timeout=self.timeout)
            can = True
        except:
            pass
        if response.status_code == 200 or can == True:
            ...
            all_list_bs = html.find(
                "div", "table-responsive fpl-list").find("tbody")
        ...
        if self.unknown == True:
            can = False
            try:
                response = requests.get("https://api.proxyscrape.com/v2/?request=displayproxies&protocol=http&timeout=5000&country=all&ssl=all&anonymity=all", headers={
                                        "User-Agent": ua}, timeout=self.timeout)
                can = True
            except:
                pass
